...
def plot_decision_boundary_2d(
    ...
    with torch.no_grad():
        y_hat = classifier.predict_proba(xy_grid).argmax(dim=1).reshape(x1_grid.shape)
    # ========================
...
def select_roc_thresh(
    ...
    # ====== YOUR CODE: ======
    fpr, tpr, thresh = roc_curve(y.detach().numpy(), classifier.predict_proba(x).detach().numpy()[:, 1])

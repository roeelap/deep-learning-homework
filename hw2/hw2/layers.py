...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        gradient = (x > 0) * (1 - self.alpha) + self.alpha
        dx = dout * gradient
        # ========================
...
class Sigmoid(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x = self.grad_cache['x']
        dx = torch.exp(x) / (torch.pow((torch.exp(x) + 1), 2))
        dx *= dout
        # ========================
...
class TanH(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        x = self.grad_cache['x']
        dx = 4 * torch.exp(2 * x) / (torch.pow((torch.exp(2 * x) + 1), 2))
        dx *= dout
        # ========================
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        correct = -x.gather(1, y.unsqueeze(1)).squeeze()
        loss = torch.mean(correct + x_softmax)
        # ========================
...
class Dropout(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        dropout = torch.bernoulli(torch.ones_like(x) * self.p)
        out = x * dropout if self.training_mode else x * (1 - self.p)
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        dx = dout * self.grad_cache['dropout'] if self.training_mode else dout * (1 - self.p)
        # ========================

...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        # ====== YOUR CODE: ======
        self.history = [0 for _, _ in params]
        self.is_first = False
        # ========================
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            if self.is_first:
                self.history[i] = self.momentum * self.history[i] - self.learn_rate * dp
            else:
                self.is_first = True
                self.history[i] = -self.learn_rate * dp
...
class RMSProp(Optimizer):
    ...
    def step(self):
        for i, (p, dp) in enumerate(self.params):
            ...
            # ====== YOUR CODE: ======
            dp += self.reg * p
            self.history[i] = self.decay * self.history[i] + (1 - self.decay) * (dp ** 2)
            p -= dp * self.learn_rate / ((self.history[i] + self.eps) ** 0.5)
            # ========================

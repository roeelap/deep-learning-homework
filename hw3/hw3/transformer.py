...
def sliding_window_attention(q, k, v, window_size, padding_mask=None):
    ...
    attention = torch.full((batch_size, seq_len, seq_len), float('-inf'), device=q.device)

    for i in range(seq_len):
        start = max(0, i - half_window)
        end = min(seq_len, i + half_window + 1)
        attention[:, i, start:end] = (q[:, i:i + 1, :] @ k[:, start:end, :].transpose(-2, -1)).squeeze(1) / torch.sqrt(
            torch.tensor(embed_dim, dtype=torch.float32))

    if padding_mask is not None:
        attention = attention.masked_fill(padding_mask.unsqueeze(1) == 0, float('-inf'))
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, padding_mask):
        ...
        # ====== YOUR CODE: ======
        attn_output, _ = self.self_attn(x, padding_mask, return_attention=True)
...
class Encoder(nn.Module):
    ...
    def forward(self, sentence, padding_mask):
        ...
        x = self.dropout(positional_encoded)

        for layer in self.encoder_layers:
            x = layer(x, padding_mask)

        cls_output = x[:, 0, :]  # Assuming [CLS] token is the first token

...
def remove_chars(text: str, chars_to_remove):
    ...
    # ====== YOUR CODE: ======
    text_clean = text
    for char in list(chars_to_remove):
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    model_input = chars_to_onehot(start_sequence, char_to_idx).unsqueeze(0).float().to(device=device)
    state = None

    with torch.no_grad():
        for _ in range(n_chars):
            output, state = model(model_input, state)
            vec = hot_softmax(output[0, -1], dim=-1, temperature=T)
            sample = torch.multinomial(vec, 1)
            model_input = idx_to_char[sample.item()]
            out_text += model_input
            model_input = chars_to_onehot(model_input, char_to_idx).unsqueeze(0).float().to(device=device)
    # ========================
...
class SequenceBatchSampler(torch.utils.data.Sampler):
    ...
    def __iter__(self) -> Iterator[int]:
        ...
        # Create a list of indices for full batches
        idx: List[int] = []
        for i in range(num_batches):
            batch_start = i * self.batch_size
            batch_end = batch_start + self.batch_size
            idx.extend(range(batch_start, batch_end))
        # ========================
        return iter(idx)

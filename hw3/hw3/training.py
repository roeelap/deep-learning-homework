...
class RNNTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        output, state = self.model(x) if self.prev_state is None else self.model(x, self.prev_state)
        ...
        self.optimizer.step()
        num_correct = (prediction.to(device=self.device) == y).sum()
        # ========================
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            prediction = torch.argmax(output, dim=2).to(device=self.device)
...
class TransformerEncoderTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        loss = torch.nn.functional.binary_cross_entropy_with_logits(logits.squeeze(), label)
        loss.backward()
        self.optimizer.step()
